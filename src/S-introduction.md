...
* [In.target: 目标读者](S-introduction.md#SS-readers)
* [In.aims: 目标](S-introduction.md#SS-aims)
* [In.not: 非目标](S-introduction.md#SS-non)
* [In.force: 强制实施](S-introduction.md#SS-force)
* [In.struct: 本文档的结构](S-introduction.md#SS-struct)
* [In.sec: 主要章节](S-introduction.md#SS-sec)

## <a name="SS-readers"></a>In.target: 目标读者

所有 C++ 程序员。其中也包括[考虑采用 C 语言的程序员](S-cpl.md#S-cpl)。
...
这些指导方针都是遵循“超集的子集”原则（[Stroustrup05](S-unclassified.md#Stroustrup05)）而设计的。
...
这些规则的设计使其能够进行[渐进式的采纳](S-modernizing.md#S-modernizing)。
...
如果您需要针对某个经验水平的教程，请参见[参考资料](S-references.md#S-references)。

本文档并不是一份如何把老旧 C++ 代码转化为更加现代的代码的指南。
而是旨在以一种具体化的方式来阐明对于新代码的设想。
当然，对于进行代码现代化转换，使其恢复活力或者升级的可行方式，可以参考[代码现代化章节](S-modernizing.md#S-modernizing)。
...
或者剖面配置中的特定规则（[type.4](S-profile.md#Pro-type-cstylecast) 或 [bounds.2](S-profile.md#Pro-bounds-arrayindex)）
...
这些信息应当出现在**替代方案**段落和[探讨](S-discussion.md#S-discussion)章节中。
...
可以在[参考资料](S-references.md#S-references)中找到一些推荐的信息来源。

...
* 使用 [RAII](S-resource.md#Rr-raii) 以避免生存期问题。
* 使用 [unique_ptr](S-functions.md#Rf-unique_ptr) 以避免生存期问题。
* 使用 [shared_ptr](S-functions.md#Rf-shared_ptr) 以避免生存期问题。
* 当不可能出现 `nullptr` 时应使用[引用](S-functions.md#Rf-ptr-ref)。
* 使用 [not_null](S-functions.md#Rf-nullptr) 以尽早捕捉到预期外的 `nullptr`。
* 使用[边界剖面配置](S-profile.md#SS-bounds)以避免范围错误。
...
这妨碍了一致性。使用 `gsl::index` 来作为下标类型；[参见 ES.107](S-expr.md#Res-subscripts)。
...
```cpp
unsigned area(unsigned height, unsigned width) { return height*width; } // [参见](S-interfaces.md#Ri-expects)
// ...
int height;
cin >> height;

...
```
注意，当你定义析构函数时，你必须定义或者弃置（delete）[所有的默认操作](S-class.md#Rc-five)：
...
```
**参见**: [有关隐式转换的讨论](S-class.md#Ro-conversion)
...
构造某个类型的实例的标准 C++ 机制是调用其构造函数。如指导方针 [C.41](S-class.md#Rc-complete) 所述：构造函数应当创建一个已完全初始化的对象。不应当需要进行如用 `memcpy` 来进行的额外初始化。
...
* [C.100: 定义容器的时候要遵循 STL](S-class.md#Rcon-stl)
* [C.101: 为容器提供值语义](S-class.md#Rcon-val)
* [C.102: 为容器提供移动操作](S-class.md#Rcon-move)
* [C.103: 为容器提供一个初始化式列表构造函数](S-class.md#Rcon-init)
* [C.104: 为容器提供一个将之置空的默认构造函数](S-class.md#Rcon-empty)
* ???
* [C.109: 当资源包装类具有指针语义时，应提供 `*` 和 `->`](S-class.md#Rcon-ptr)

**参见**: [资源](S-resource.md#S-resource)
...
函数对象应当可廉价地复制（因此可以[按值传递](S-functions.md#Rf-in)）。

概要：

* [F.10: 若操作可被重用，则应为其命名](S-functions.md#Rf-name)
* [F.11: 当需要仅在一处使用的简单函数对象时使用无名 lambda]([#Rf-lambda)
* [F.50: 无法用函数达成（捕捉局部变量，或者编写局部函数）时，应使用 lambda](S-functions.md#Rf-capture-vs-overload)
* [F.52: 在将被局部范围内使用（包括将之传递给算法）的 lambda 中优先按引用捕捉](S-functions.md#Rf-reference-capture)
* [F.53: 在不被局部范围内使用（包括存储在堆上，或传递给其他线程）的 lambda 中避免按引用捕捉](S-functions.md#Rf-value-capture)
* [ES.28: 针对复杂的初始化（尤其是 `const` 变量）使用 lambda](S-expr.md#Res-lambda-init)
...
* [C.120: 类层次（仅）用于表达具有天然层次化结构的概念](S-class.md#Rh-domain)
* [C.121: 如果基类被用作接口的话，应使其成为纯抽象类](S-class.md#Rh-abstract)
* [C.122: 当需要完全区分接口和实现时，应当用抽象类作为接口](S-class.md#Rh-separation)

类层次的设计规则概览：

* [C.126: 抽象类通常并不需要用户编写的构造函数](S-class.md#Rh-abstract-ctor)
* [C.127: 带有虚函数的类应当带有虚的或受保护的析构函数](S-class.md#Rh-dtor)
* [C.128: 虚函数应当指明 `virtual`、`override`、`final` 三者之一](S-class.md#Rh-override)
* [C.129: 当设计类层次时，应区分实现继承和接口继承](S-class.md#Rh-kind)
* [C.130: 多态类的深拷贝；优先采用虚函数 `clone` 来替代公开复制构造/赋值](S-class.md#Rh-copy)
* [C.131: 避免无价值的取值和设值函数](S-class.md#Rh-get)
* [C.132: 请勿无理由地使函数 `virtual`](S-class.md#Rh-virtual)
* [C.133: 避免 `protected` 数据](S-class.md#Rh-protected)
* [C.134: 确保所有非 `const` 数据成员有相同的访问级别](S-class.md#Rh-public)
* [C.135: 用多继承来表达多个不同的接口](S-class.md#Rh-mi-interface)
* [C.136: 用多继承来表达一些实现特性的合并](S-class.md#Rh-mi-implementation)
* [C.137: 用 `virtual` 基类以避免过于通用的基类](S-class.md#Rh-vbase)
* [C.138: 用 `using` 来为派生类和其基类建立重载集合](S-class.md#Rh-using)
* [C.139: 对类运用 `final` 应当保守](S-class.md#Rh-final)
* [C.140: 不要在虚函数和其覆盖函数上提供不同的默认参数](S-class.md#Rh-virtual-default-arg)

对类层次中的对象进行访问的规则概览：

* [C.145: 通过指针和引用来访问多态对象](S-class.md#Rh-poly)
* [C.146: 当无法避免在类层次上进行导航时应使用 `dynamic_cast`](S-class.md#Rh-dynamic_cast)
* [C.147: 当查找所需类的失败被当做一种错误时，应当对引用类型使用 `dynamic_cast`](S-class.md#Rh-ref-cast)
* [C.148: 当查找所需类的失败被当做一种有效的可能情况时，应当对指针类型使用 `dynamic_cast`](S-class.md#Rh-ptr-cast)
* [C.149: 用 `unique_ptr` 或 `shared_ptr` 来避免忘记对以 `new` 所创建的对象进行 `delete` 的情况](S-class.md#Rh-smart)
* [C.150: 用 `make_unique()` 来构建由 `unique_ptr` 所拥有的对象](S-class.md#Rh-make_unique)
* [C.151: 用 `make_shared()` 来构建由 `shared_ptr` 所拥有的对象](S-class.md#Rh-make_shared)
* [C.152: 禁止把指向派生类对象的数组的指针赋值给指向基类的指针](S-class.md#Rh-array)
* [C.153: 优先采用虚函数而不是强制转换](S-class.md#Rh-use-virtual)
...
带有虚函数的类通常是通过指向基类的指针来使用的。一般来说，最后一个使用者必须在基类指针上执行 `delete`，这常常是通过基类智能指针来做到的，因而析构函数应当为 `public` 和 `virtual`。而不那么常见的情况是当并不打算支持通过基类指针来删除时，这时析构函数应当为 `protected` 和非 `virtual`；参见 [C.35](S-class.md#Rc-dtor-virtual)。
...
纯粹的接口类只是一组纯虚函数；参见 [I.25](S-interfaces.md#Ri-abstract)。
...
分离接口和实现的另一个（相关的）技巧是 [Pimpl](S-interfaces.md#Ri-pimpl)。
...
由于切片的问题，不鼓励多态类的复制操作，参见 [C.67](S-class.md#Rc-copy-virtual)。如果确实需要复制语义的话，应当进行深复制：提供一个虚 `clone` 函数，它复制的是真正的最终派生类型，并返回指向新对象的具有所有权的指针，而且在派生类中它返回的也是派生类型（利用协变返回类型）。
...
```
通常来说，推荐使用智能指针来表示所有权（参见 [R.20](S-resource.md#Rr-owner)）。不过根据语言规则，协变返回类型不能是智能指针：当 `B::clone` 返回 `unique_ptr<B>` 时 `D::clone` 不能返回 `unique_ptr<D>`。因此，你得在所有覆盖函数中统一都返回 `unique_ptr<B>`，或者也可以使用[指导方针支持库](S-gsl.md#SS-views)中的 `owner<>` 工具类。
...
```
注意，我们可以为成员变量提供默认初始化式：[C.49: 优先进行初始化而不是在构造函数中赋值](S-class.md#Rc-initialize).
...
应当[优先采用 `private` 数据](S-class.md#Rc-private)并提供良好定义并强加的不变式。
或者通常更好的做法是，[不要在用作接口的任何类中存放数据](S-class.md#Rh-abstract)。
...
* *全 public*: 如果编写的是聚集一组变量而没有在这些变量之间维护不变式的话，所有这些变量都应当为 `public`。
  [依照惯例，应当把这样的类声明为 `struct` 而不是 `class`](S-class.md#Rc-struct)
...
`Interface` 是一个[接口层次](S-class.md#Rh-abstract)的根，
而 `Utility` 则是一个[实现层次](S-class.md#Rh-kind)的根。
...
[多态类应当抑制复制操作](S-class.md#Rc-copy-virtual)
...
[优先使用虚函数而不是强制转换](S-class.md#Rh-use-virtual)。
...
* 本条规则属于[类型安全性剖面配置](S-profile.md#Pro-type-downcast)。
...
与此相对，[C.147](S-class.md#Rh-ref-cast) 中失败即是错误，而且不能用于条件执行。
...
参见 [R.23](S-resource.md#Rr-make_unique)。

### <a name="Rh-make_shared"></a>C.151: 用 `make_shared()` 来构建由 `shared_ptr` 所拥有的对象

参见 [R.22](S-resource.md#Rr-make_shared)。
...
参见 [C.146](S-class.md#Rh-dynamic_cast) 和 ???
...
* [C.160: 定义运算符应当主要用于模仿传统用法](S-class.md#Ro-conventional)
* [C.161: 对于对称的运算符应当采用非成员函数](S-class.md#Ro-symmetric)
* [C.162: 重载的操作之间应当大体上是等价的](S-class.md#Ro-equivalent)
* [C.163: 应当仅对大体上等价的操作进行重载](S-class.md#Ro-equivalent-2)
* [C.164: 避免隐式转换运算符](S-class.md#Ro-conversion)
* [C.165: 为定制点采用 `using`](S-class.md#Ro-custom)
* [C.166: 一元 `&` 的重载只能作为某个智能指针或引用系统的一部分而提供](S-class.md#Ro-address-of)
* [C.167: 应当为带有传统含义的操作提供运算符](S-class.md#Ro-overload)
* [C.168: 应当在操作数所在的命名空间中定义重载运算符](S-class.md#Ro-namespace)
* [C.170: 当想要重载 lambda 时，应当使用泛型 lambda](S-class.md#Ro-lambda)
...
```
这里维持了传统的语义：[副本之间相等](S-class.md#SS-copy)。
...
非成员运算符应当要么是友元，要么定义于[其操作数所在的命名空间中](S-class.md#Ro-namespace)。
[二元运算符应当等价地对待其两个操作数](S-class.md#Ro-symmetric)。
...
这是这条规则的一种特殊情况：[辅助函数应当定义于它们的类相同的命名空间之中](S-class.md#Rc-helper)。
...
* [C.180: 采用 `union` 用以节省内存](S-class.md#Ru-union)
* [C.181: 避免“裸” `union`](S-class.md#Ru-naked)
* [C.182: 利用匿名 `union` 实现带标记联合体](S-class.md#Ru-anonymous)
* [C.183: 不要将 `union` 用于类型双关](S-class.md#Ru-pun)
* ???
...
cout << v.d << '\n';    // 写下 987.654
```
但请你留意这个警告：[避免“裸”`union`](S-class.md#Ru-naked)。

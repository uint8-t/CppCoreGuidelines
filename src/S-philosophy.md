...
* [P.1: 在代码中直接表达你的想法](S-philosophy.md#Rp-direct)
* [P.2: 用 ISO 标准 C++ 来编码](S-philosophy.md#Rp-Cplusplus)
* [P.3: 表达你的设计意图](S-philosophy.md#Rp-what)
* [P.4: 理想情况下，程序应当是静态类型安全的](S-philosophy.md#Rp-typesafe)
* [P.5: 编译期检查优先于运行时检查](S-philosophy.md#Rp-compile-time)
* [P.6: 应当使无法在编译期进行的检查能够在运行时实施](S-philosophy.md#Rp-run-time)
* [P.7: 尽早识别运行时错误](S-philosophy.md#Rp-early)
* [P.8: 不要泄漏任何资源](S-philosophy.md#Rp-leak)
* [P.9: 不要浪费时间或空间](S-philosophy.md#Rp-waste)
* [P.10: 不可变数据优先于可变数据](S-philosophy.md#Rp-mutable)
* [P.11: 把杂乱的构造封装起来，而别让其散布到代码中](S-philosophy.md#Rp-library)
* [P.12: 适当采用支持工具](S-philosophy.md#Rp-tools)
* [P.13: 适当采用支持程序库](S-philosophy.md#Rp-lib)
...
应当避免依赖于未定义的行为（例如，[未定义的求值顺序](S-expr.md#Res-order)）
...
`for` 语句的更多细节，请参见 [ES.71](S-expr.md#Res-for-range)。
...
建议采用 [RAII](S-resource.md#Rr-raii)：
...
**参见**: [资源管理相关章节](S-resource.md#S-resource)
...
强制实行[生存期安全性剖面配置](S-profile.md#SS-lifetime)可以消除泄漏的发生。
如果和 [RAII](S-resource.md#Rr-raii) 所提供的资源安全性组合到一起，也可以（通过不产生任何垃圾而）消除对“垃圾收集”的需要。
如果将之和[类型和边界剖面配置](S-introduction.md#SS-force) 组合到一起强制实施的话，你将会得到完全的类型和资源安全性，这是通过使用工具来保证的。
...
另见 [Con: 常量和不可变性](S-const.md#S-const)
...
来面对需要重复把低级代码搞正确的挑战。
这是作为指导方针基石的[超集的子集原则](S-introduction.md#R0)的一种变体。
...
* [静态分析工具](???)
* [并发工具](S-concurrency.md#Rconc-tools)
* [测试工具](???)
